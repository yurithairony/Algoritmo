...
void append_int(int *pointer, int *lenght, int capacity, int n, int *erro) {
  *erro = 1;
  if (*lenght != capacity) {
    *(pointer+*lenght)=n;
    lenght++;
...
int insert_int(int l[], int lenght, int capacity, int x, int index) {
  if (lenght!=capacity) {
    int i;
    l[i] = l[i-1];
  }
...
array_list_int *array_list_create() {
  ...
  /* Initial size: 8*/
  if (new_list->data == 0) { 
    /* Could not allocate memory for list data */
    free(new_list);
  ...
  new_list->capacity = 8;
  return new_list; 
  /* Returns pointer to new allocated array_list */
}
